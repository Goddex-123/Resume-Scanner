...
DEMO_RESUME_PATH = os.path.join(
    os.path.dirname(__file__), "samples", "sample_resume.txt"
)
...
<div class="analysis-card-title">📑 Resume Sections</div>
        ...
        for key, label in section_labels.items():
            ...
            icon = (
                ...
                if found
                else '<span class="check-missing">✗</span>'
            )
...
<strong>ATS Pass:</strong> {'✅ Likely' if ats_results.get('pass_ats') else '❌ Unlikely'}
                ...
                if suggestions:
                    st.markdown(
                        '<h3 class="section-header">🎯 Improvement Suggestions</h3>',
                        unsafe_allow_html=True,
                    )
...
<div class="suggestion-text">{sug}</div>
    ...
    with tab3:
        if run_ai and ai_results:
            ...
            with col_a:
                st.plotly_chart(
                    create_gauge_chart(ai_prob, "AI Content Score"),
                    use_container_width=True,
                )
                ...
                if detailed:
                    st.plotly_chart(
                        create_ai_breakdown_chart(detailed), use_container_width=True
                    )
...
<div style="color:#94a3b8;">Confidence: <strong>{confidence}</strong></div>
                ...
                if ai_results.get("flags"):
                    st.markdown(
                        '<h4 style="color:#f59e0b; margin-top:18px;">⚠️ Flags Detected</h4>',
                        unsafe_allow_html=True,
                    )
                    for flag in ai_results["flags"]:
                        st.markdown(
                            f'<div class="feedback-item feedback-warning">{flag}</div>',
                            unsafe_allow_html=True,
                        )
                ...
                if detailed:
                    st.markdown(
                        '<h3 class="section-header">📊 Sub-Score Breakdown</h3>',
                        unsafe_allow_html=True,
                    )
                    for key, val in detailed.items():
                        ...
                        color = (
                            "#ef4444"
                            if val >= 60
                            else "#f59e0b" if val >= 35 else "#10b981"
                        )
    ...
    with tab4:
        if run_jobs and job_results:
            ...
            if matches:
                st.plotly_chart(
                    create_job_match_chart(matches), use_container_width=True
                )
...
<div style="font-size:1.4rem; color:#8b5cf6; font-family:'JetBrains Mono',monospace;">
                ...
                if recs:
                    st.markdown(
                        '<h3 class="section-header">💡 Recommendations</h3>',
                        unsafe_allow_html=True,
                    )
...
<div class="suggestion-text">{rec}</div>
    ...
    with tab5:
        ...
        with di1:
            ...
            st.plotly_chart(
                create_text_quality_chart(text_quality), use_container_width=True
            )
...
<div class="analysis-card-title">📊 Document Statistics</div>
            ...
            if found_verbs:
                ...
                badges = "".join(
                    [
                        f'<span class="skill-badge" style="border-color:rgba(16,185,129,0.3); font-size: 0.75rem; padding: 2px 6px; margin: 2px;">{v.title()}</span>'
...
<div class="analysis-card-title">📐 Formatting & Structure</div>
            ...
            bp_icon, bp_color, bp_text = (
                ("✅", "#10b981", "Uses Bullet Points")
                if has_bullets
                else ("❌", "#ef4444", "No Bullet Points Detected")
            )
...
<span style="color:{bp_color}; font-weight:500;">{bp_text}</span>
        ...
        with di2:
            ...
            if run_ats and ats_results:
                st.markdown(
                    '<h3 class="section-header">🔑 Keyword Coverage</h3>',
                    unsafe_allow_html=True,
                )
                ...
                if found_kw:
                    st.markdown(
                        f'<div class="category-title">✅ Found ({len(found_kw)})</div>',
                        unsafe_allow_html=True,
                    )
                    badges = "".join(
                        [
                            f'<span class="skill-badge" style="border-color:rgba(16,185,129,0.3);">{k.title()}</span>'
                            for k in found_kw
                        ]
                    )
                    st.markdown(badges, unsafe_allow_html=True)
                ...
                if missing_kw:
                    st.markdown(
                        f'<br><div class="category-title">❌ Missing ({len(missing_kw)})</div>',
                        unsafe_allow_html=True,
                    )
                    badges = "".join(
                        [
                            f'<span class="skill-badge" style="border-color:rgba(239,68,68,0.3); opacity:0.7;">{k.title()}</span>'
                            for k in missing_kw
                        ]
                    )
                    st.markdown(badges, unsafe_allow_html=True)
...
<div class="analysis-card-title">📖 Vocabulary Analysis</div>
            ...
            ttr_color = (
                "#10b981" if ttr >= 0.5 else "#f59e0b" if ttr >= 0.35 else "#ef4444"
            )
            ...
            verb_color = (
                "#10b981"
                if verb_pct >= 2.5
                else "#f59e0b" if verb_pct >= 1.0 else "#ef4444"
            )
...
<div class="hero-badge">✦ v2.0 — Premium Edition</div>
    ...
    with st.sidebar:
        ...
        st.markdown('<div class="styled-divider"></div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="sidebar-title">🔮 Powered By</div>', unsafe_allow_html=True
        )
        st.markdown(
            """
<div>
...
<div class="upload-hint">Supports PDF and DOCX • Up to 1 GB</div>
    ...
    if uploaded_file:
        ...
        try:
            ...
            st.info(
                "💡 Make sure the file is a valid PDF or DOCX and is not corrupted."
            )
            return
        ...
        try:
            with open(DEMO_RESUME_PATH, "r", encoding="utf-8") as f:
                text = f.read()
            st.info(
                "📄 Analyzing the built-in **demo resume** — upload your own to get personalized results!"
            )
        except FileNotFoundError:
            st.error(
                "❌ Demo resume file not found. Please ensure `samples/sample_resume.txt` exists."
            )
            return
