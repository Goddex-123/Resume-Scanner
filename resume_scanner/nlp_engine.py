...
class NLPEngine:
    ...
    def calculate_experience_years(self, text: str) -> Tuple[float, List[Dict]]:
        ...
        for start, end in matches:
            ...
            if (
                1990 <= start_year <= current_year
                and start_year <= end_year <= current_year
            ):
                years = end_year - start_year
                total_years += years
                experiences.append(
                    {"start": start_year, "end": end_year, "years": years}
                )
    ...
    def analyze_text_quality(self, text: str) -> Dict[str, float]:
        ...
        found_action_verbs = list(
            set([w.lower() for w in words if w.lower() in action_verbs])
        )
