...
def create_skill_radar(skills_dict):
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            marker=dict(size=7, color="#06b6d4", line=dict(color="#0a0a1a", width=2)),
...
def create_text_quality_chart(quality_metrics):
    ...
    metrics = {
        "Vocabulary Richness": min(
            quality_metrics.get("vocabulary_richness", 0) * 200, 100
        ),
        "Action Verb Usage": min(
            quality_metrics.get("action_verb_percentage", 0) * 10, 100
        ),
        "Sentence Clarity": max(
            0, 100 - abs(quality_metrics.get("avg_sentence_length", 20) - 20) * 3
        ),
